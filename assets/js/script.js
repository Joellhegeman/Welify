...
function deletePlaylist(playlistId) {
    ...
    if(prompt == true){
        $.post("includes/handlers/ajax/deletePlaylist.php",{playlistId: playlistId}).done(function(error) {
            ...
            //.done is een defferends, done is een prefered way voor reageren om een ajax call
...
function formatTime(seconds) {
    ...
    var minutes = Math.floor(time / 60); // Rond nummer af
    var seconds = time - (minutes * 60);

    var extraZero;  //var extraZero = (seconds < 10) ? "0" : "";
...
function Audio() {
    ...
    this.audio.addEventListener("canplay", function () {
        //'this' refers to the object that ehe event was called on
